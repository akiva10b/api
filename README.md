...
- 🎯 Entry points to execute your program using `python -m <api>` or `$ api` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/akiva10b/api/branch/main/graph/badge.svg?token=api_token_here)](https://codecov.io/gh/akiva10b/api)
[![CI](https://github.com/akiva10b/api/actions/workflows/main.yml/badge.svg)](https://github.com/akiva10b/api/actions/workflows/main.yml)

Awesome api created by akiva10b

## Install

from source
```bash
git clone https://github.com/akiva10b/api api
cd api
make install
```
...
```bash
❯ api create-user --help
Usage: api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
api create-user admin admin --superuser
```
...
Run with `api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `api create-user`.
...
api/db.py                   10      0   100%
api/models/__init__.py       0      0   100%
api/models/content.py       47      1    98%
api/routes/__init__.py      11      0   100%
api/routes/content.py       52     25    52%
api/routes/security.py      15      1    93%
api/routes/user.py          52     26    50%
api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
export api_KEY="@int 42"
export api_KEY="@jinja {{ this.db.uri }}"
export api_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
