...
fmt:              ## Format code using black & isort.
	$(ENV_PREFIX)isort api/
	$(ENV_PREFIX)black -l 79 api/
	$(ENV_PREFIX)black -l 79 tests/

.PHONY: lint
lint:             ## Run pep8, black, mypy linters.
	$(ENV_PREFIX)flake8 api/
	$(ENV_PREFIX)black -l 79 --check api/
...
release:          ## Create a new tag for release.
	...
	@git add api/VERSION HISTORY.md
...
switch-to-poetry: ## Switch to poetry package manager.
	...
	@echo "api = 'api.__main__:main'" >> pyproject.toml
	...
	@echo "Please run 'poetry shell' or 'poetry run api'"
...
shell:            ## Open a shell in the project.
	...
	@./.venv/bin/ipython -c "from api import *"

.PHONY: docker-build
docker-build:	  ## Builder docker images
	@docker-compose -f docker-compose-dev.yaml -p api build

.PHONY: docker-run
docker-run:  	  ## Run docker development images
	@docker-compose -f docker-compose-dev.yaml -p api up -d

.PHONY: docker-stop
docker-stop: 	  ## Bring down docker dev environment
	@docker-compose -f docker-compose-dev.yaml -p api down

.PHONY: docker-ps
docker-ps: 	  ## Bring down docker dev environment
	@docker-compose -f docker-compose-dev.yaml -p api ps

.PHONY: docker-log
docker-logs: 	  ## Bring down docker dev environment
	@docker-compose -f docker-compose-dev.yaml -p api logs -f app
